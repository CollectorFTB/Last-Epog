...
def save_idols():
    ...
    saved_idols = []
    for idol, prefix_index, suffix_index, preifx_roll, suffix_roll in idol_screen.locked_idols:
        saved_idols.append((idol_screen.all_idols.index(idol), prefix_index, suffix_index, preifx_roll, suffix_roll))
...
def load_idols(idols_save):
    ...
    for idol_index, prefix_index, suffix_index, prefix_roll, suffix_roll in loaded_idols:
        idol_screen.locked_idols.append((idol_screen.all_idols[idol_index], prefix_index, suffix_index, prefix_roll, suffix_roll))
    ...
    for i in range(len(grid)):
        for j in range(len(grid[i])):
            if (wr := grid[i][j]) not in [0, 1]:
                idol, _, _, _, _ = idol_screen.locked_idols[int(wr)]
                if wr not in passed:
                    passed.append(wr)
